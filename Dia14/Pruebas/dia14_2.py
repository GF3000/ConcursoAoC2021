...
def contar_letras(contador_duplas, contador):
    ...
    for letra in list(contador.keys()):
        if (contador[letra] %2 == 0):#Si es par es que se ha sumado dos veces (no es ni inicial ni final)
            contador[letra] = int(contador[letra] /2 )
        else:#Si es inical o final será impar, se suma el extremo  y el resto /2 que se han contado dos veces
            contador[letra] = 1 + int(contador[letra] /2 )
    return contador
