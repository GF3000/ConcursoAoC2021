...
def es_recta_diagonal(recta):
    if abs(recta[2]-recta[0]) == abs(recta[3]-recta[1]):
        return True
    else:
        return False
...
def puntos_por_rectas (recta):
    ...
    elif(es_recta_diagonal(recta)):
        direccion_x = int((recta[2] - recta[0]) / abs(recta[2] - recta[0]))
        direccion_y = int((recta[3] - recta[1]) / abs(recta[3] - recta[1]))
        longitud = abs(recta[3]-recta[1])
        for puntitos in range(longitud+1):
            puntos_ocupados.append([recta[0]+direccion_x*puntitos,recta[1]+direccion_y*puntitos])
